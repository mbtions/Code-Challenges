class Solution 
{
    public int[] twoSum(int[] numbers, int target) 
    {
        ...
        indices = new int [2]; //since we have to only return 2 indexes
         if(numbers.length < 2)
         {
             return null; // if array has less than 2 numbers then no sum hence no target
         }
          ...
          for(int i = 0; i < numbers.length; i++)
          {
              ...
              {
                   if(numbers[i]+numbers[j] == target && i < j)  // if index1<index2 condition is also followed along with the getting target
                   {
                        indices[0] = i+1;
                        indices[1] = j+1;
